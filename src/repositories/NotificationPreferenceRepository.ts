...
export class NotificationPreferenceRepository {
  ...
  private async create(preference: NotificationPreference) {
    ...
    const params = [preference.id, preference.churchId, preference.personId, preference.allowPush, preference.emailFrequency];
  ...
  private async update(preference: NotificationPreference) {
    ...
    const params = [preference.personId, preference.allowPush, preference.emailFrequency, preference.id, preference.churchId]
    await DB.query(sql, params)
    return preference;
  }
