...
export class BlockedIpRepository {
    ...
    public async save(blockedIp: BlockedIp) {
        const existingIp = await DB.query("SELECT id FROM blockedIps WHERE churchId=? AND conversationId=? AND ipAddress=?;", [blockedIp.churchId, blockedIp.conversationId, blockedIp.ipAddress]);
    ...
    public convertToModel(data: any) {
        const result: BlockedIp = { id: data.id, churchId: data.churchId, conversationId: data.conversationId, serviceId: data.serviceId, ipAddress: data.ipAddress };
        return result;
    }
